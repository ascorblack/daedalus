...
export function ProjectSidebar(p: ProjectSidebarProps) {
  ...
  const { team, board, terminals, sessions, schedules } = useFocus(p.projectId);
  ...
  const wakeups = orchestratorId ? schedules.filter((s) => s.target_session === orchestratorId && s.enabled).length : 0;
