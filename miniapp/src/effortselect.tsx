...
export function EffortOptions({ effort, thinking, onChoose }: Omit<EffortSelectProps, "model">) {
  ...
  return <fieldset className="effort-options">
    ...
    {["off", ...REASONING_EFFORTS].map((value) => <label key={value} className="effort-option">
      <input type="radio" name={name} value={value} checked={value === "off" ? !thinking : !!thinking && value === current} onChange={() => onChoose(value)} />
