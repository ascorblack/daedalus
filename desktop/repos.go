...
func EnsureRepos(ctx context.Context, p Paths, log func(string, ...any)) error {
	...
	for _, repo := range []struct {
		...
		if _, err := runDocker(ctx, gitArgs(p, "clone", "--depth", cloneDepth, repo.remote, "/work/"+repo.name)...); err != nil {
			return err
		}
	}
	return nil
...
func UpdateRepos(ctx context.Context, p Paths, log func(string, ...any)) error {
	for _, repo := range []struct {
		...
		if _, err := runDocker(ctx, gitArgs(p, "-C", "/work/"+repo.name, "fetch", "--depth", cloneDepth, "origin", "main")...); err != nil {
			return err
		}
		if _, err := runDocker(ctx, gitArgs(p, "-C", "/work/"+repo.name, "reset", "--hard", "origin/main")...); err != nil {
			return err
		}
	}
	return nil
