...
1. Run a Lighthouse accessibility audit from the shell (`npx lighthouse <url> --only-categories=accessibility --chrome-flags="--headless --no-sandbox" --output=json --output-path=./a11y.json`) and read the failing audits from the JSON. Use mobile emulation for a general public page.
...
Run Lighthouse from the shell: `npx lighthouse` downloads on first use and drives the Chromium bundled with Playwright (`CHROME_PATH` is preset on this host). Then:
