...
`assets/tikz-preamble.tex` in this skill's directory defines the block, data,
arrow, and stage styles and the same palette the plots use (the `Skill` tool
prints where the directory is). Copy it as the head of the figure's `.tex`:

```sh
mkdir -p figs && cat <skill-dir>/assets/tikz-preamble.tex > figs/method.tex
```

A `.tex` compiles where it sits, so `figs/method.tex` produces `figs/method.pdf`
next to it. Compile it with whatever TeX the workspace has (`latexmk -pdf
figs/method.tex`, or `tectonic figs/method.tex`); install one with the package
manager if there is none.
