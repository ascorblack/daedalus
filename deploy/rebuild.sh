...
while true; do
    ...
    if [ -f "$trigger/dependencies-request" ]; then
        ...
        if docker compose -f "$COMPOSE_FILE" --env-file .env build daedalus > "$trigger/dependencies-build.log" 2>&1; then
            if docker compose -f "$COMPOSE_FILE" --env-file .env up -d --no-build daedalus >> "$trigger/dependencies-build.log" 2>&1; then
                result=completed
            else
