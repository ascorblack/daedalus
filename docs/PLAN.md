...
- `rebuild`: сообщает боту «сохрани снапшоты» → `git fetch && git checkout origin/main` в
  `/srv/daedalus` и `/srv/protocore` → если изменились `Dockerfile`/`pyproject`/`uv.lock` —
...
`selfdev/git.py`: агент работает в ветке `agent/<slug>` в `/srv/daedalus` или `/srv/protocore`;
