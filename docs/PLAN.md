...
**Выполнен** (2026-09-06; приёмка по единицам — в `ACCEPTANCE.md`). Хранится как запись порядка сборки;
текущее устройство репозитория и имена модулей — в `README.md` («Layout»), дальнейшие изменения — в
`CHANGELOG.md`.
